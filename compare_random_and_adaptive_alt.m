function compare_random_and_adaptive_alt(dim1,dim2,res,N,horizon)
...
for num_obj = 2:N  
    ...
    for a = 1:2
        ...
        for i = 1:num
            ...
            if (a == 1)
                ...
                [n_iterations(i,a),n_actions(i,a),time_taken(i,a),n_unknown(i,a)] = ...
                    adaptive_horizon_search_alt(dim1,dim2,res,objLocationsIndices,[],1,horizon,0);
            else
                [n_iterations(i,a),n_actions(i,a),time_taken(i,a),n_unknown(i,a)] = ...
                    random_search_alt(dim1,dim2,res,objLocationsIndices,1,horizon,0);
            end
            if (n_actions(i,a) < 1000)
                successful(a,1) = successful(a,1)+1;
            else
                n_iterations(i,a) = 0;
                n_actions(i,a) = 0;
                time_taken(i,a) = 0;
        ...
        avg_n_iterations(num_obj-1,a) = sum(n_iterations(:,a))/successful(a,1);
        avg_n_actions(num_obj-1,a) = sum(n_actions(:,a))/successful(a,1);
        avg_time_taken(num_obj-1,a) = sum(time_taken(:,a))/successful(a,1);
        avg_n_unknown(num_obj-1,a) = sum(n_unknown(:,a))/successful(a,1);
    end
end
