function random_action_seq = random_plan(visible,known,occupied,free,dim1,dim2,res,horizon)
...
if (isempty(allowed_actions) == 0)
    ...
    [~,~,~,new_visible,new_known,new_occupied,new_free,~,~] = ...
        simulate_action2(old_config,random_action,true_config,visible,known,occupied,free,nrows,ncols);
    
    if (horizon > 1)
        action_seq = random_plan(new_visible,new_known,new_occupied,new_free,dim1,dim2,res,horizon-1);
        horizon;
    else
        action_seq = [];
    end
