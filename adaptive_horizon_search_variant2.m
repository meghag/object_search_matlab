function [iteration,n_actions_taken,time_taken,unknown_state] = adaptive_horizon_search_variant2(dim1,dim2,res,objLocationsindices,target_id,horizon,plot_flag)
% A variant of adaptive horizon search
% Goal: reveal state of ALL voxels even if horizon length has to be increased.
% Replan with the current  value of horizon in case of new findings.
    ...
    for act = 1:size(action_sequence,1)
       ...
       % known_before_action = known;
        ...
        if (no_new_findings == 0)
            %Something new was seen that we didn't expect to see.
