%function [iteration,n_actions_taken,time_taken,unknown_state] = fixed_horizon_search_alt(dim1,dim2,res,objLocationsindices,knownLocationsIndices,target_id,horizon,plot_flag)
function log_cell_array = fixed_horizon_search_alt(dim1,dim2,res,objLocationsindices,knownLocationsIndices,target_id,horizon,plot_flag)
% ADAPTIVE_HORIZON
...
%for itr = 1:1
    ...
    tstart_planning = tic;
    [info_gain,action_sequence] = plan2_alt(visible,known,occupied,free,dim1,dim2,res,horizon,0);
    if (isempty(action_sequence))
        disp('No more moves possible.');
        time_taken = toc(tstart_search);
        iteration_details = [iteration_details, [horizon; 0; toc(tstart_planning)]];
    ...
    horizon_distribution(horizon) = horizon_distribution(horizon)+1;
    ...
    for act = 1:size(action_sequence,1)
       ...
       % known_before_action = known;
        ...
        if (nrows*ncols - (length(occupied) + length(free)) == 0)
            time_taken = toc(tstart_search);
            iteration_details = [iteration_details, [horizon; act; planning_time_taken]];
            unknown_state = 0;
            log_cell_array = {iteration,n_actions_taken,time_taken,unknown_state, horizon_distribution, iteration_details};
            return;
        end
