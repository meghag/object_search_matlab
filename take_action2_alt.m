function [new_config,new_true_config,moved,visible,known,occupied,free,revealed,no_new_findings] = ...
    take_action2_alt(old_config,action,true_config,visible,known,occupied,free,nrows,ncols)
...
if (action(1) >= 1000)
    %disp('Object id > 1000');
    [~,idx1,~] = intersect(true_config(:,2:3),action(2:3),'rows');
...
[~,move_idx,~] = intersect(old_config(:,2:3),action(2:3),'rows');
