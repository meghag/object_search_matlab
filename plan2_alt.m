function [info_gain, best_next_action_seq] = plan2_alt(visible,known,occupied,free,dim1,dim2,res,horizon,prev_info_gain)
...
allowed_actions = find_actions2_alt(visible,occupied,prev_free,dim1,dim2,res);
...
for i = 1:size(allowed_actions,1)
    ...
    destination_cell = allowed_actions(i,4:5);
...
%     end
    ...
    [~,~,~,new_visible,new_known,new_occupied,new_free,~,~] = ...
        simulate_action2_alt(old_config,allowed_actions(i,:),true_config,known,occupied,free,nrows,ncols);
...
%     end
    ...
    if (horizon > 1)
        [total_gain_so_far, best_next_action_seq] = ...
            plan2_alt(new_visible,new_known,new_occupied,new_free,dim1,dim2,res,horizon-1,total_gain_so_far);
        horizon;
    else 
