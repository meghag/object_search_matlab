function plot_config2_qual(config,visible,known,occupied_indices,free,dim1,dim2,res,title_text,target_id)
...
for i = 1:dim1/res
    for j = 1:dim2/res
        ...
        %plot(cellCentres(i,j,1),cellCentres(i,j,2),'-rs','MarkerSize',110, 'MarkerFaceColor',[0.5,0.5,0.5],'LineWidth', 2);
    end
end
...
for i = 1:size(unknown,1)
    ...
    plot(centre(1),centre(2),'-ks','MarkerSize',110, 'MarkerFaceColor','k','LineWidth', 2);
end

[free_idx(:,1),free_idx(:,2)] = ind2sub([dim1/res,dim2/res],free);
for i = 1:size(free,1)
    % Never seen or felt objects
    centre = cellCentres(free_idx(i,1),free_idx(i,2),:);
    plot(centre(1),centre(2),'-rs','MarkerSize',110, 'MarkerFaceColor','w','LineWidth', 2);
end
...
%         plot(centre(1),centre(2),'-d','MarkerFaceColor',color{config(i,1)},'MarkerSize',50);
    if (isempty(find(config(i,1) == visible(:,1), 1)))
        ...
        if (numel(find(config(i,1) == known(:,1))) ~= 0)
            ...
            plot(centre(1),centre(2),'-sr','MarkerFaceColor',[0.5,0.5,0.5],'MarkerSize',110);
            plot(centre(1),centre(2),'-ok','MarkerFaceColor',color{config(i,1)},'MarkerSize',30);
        %circle(centre,res/4,1000,'-',color{config(i,1)});
        else
            % The object is unknown
            %plot(centre(1),centre(2),'-s','MarkerFaceColor','k','MarkerSize',100);
            plot(centre(1),centre(2),'-ok','MarkerFaceColor',color{config(i,1)},'MarkerSize',30);
        end
    else
        plot(centre(1),centre(2),'-rs','MarkerSize',110, 'MarkerFaceColor','w','LineWidth', 2);
