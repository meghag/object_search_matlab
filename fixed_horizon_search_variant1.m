function [iteration,n_actions_taken,time_taken,unknown_state] = fixed_horizon_search_variant1(dim1,dim2,res,objLocationsindices,target_id,horizon,plot_flag)
% A variant of fixed horizon search
% Goal: reveal as much unknown space as possible without increasing
% horizon length. Let us see how the horizon length limits the extent to
% which one can observe the space.
...
% unknown_state - no. of voxels whose state could not be revealed.
...
%for itr = 1:1
    ...
    info_gain;
    if (info_gain < 1)
        if temp_horizon == horizon
            %Calculate number of unknown state voxels
    ...
    for act = 1:size(action_sequence,1)
       ...
       % known_before_action = known;
        ...
        if (moved == 0)
            %Replan.
            remaining_steps_in_plan = size(action_sequence,1) - act;
            temp_horizon = remaining_steps_in_plan;
            break;
        end
        ...
        %else
            %Yes, everything is as expected. Continue executing actions. 
        if (act == size(action_sequence,1))
            temp_horizon = horizon;
        end
    end
end
